...
| The trademark-notice wording is not in the repository — it comes from outside it (counsel)                                                                                   | `LICENSE` is plain MIT with no trademark clause; no `TRADEMARK` file exists. The wording is what `.works/works.yml` `license.notice` carries                                                                                                                                                             |
...
2. **Resolved: build services are reached on `127.0.0.1` with throwaway credentials.** APW-07 plan §4.6.2 gives the
   ...
   than as a literal (see `.works/works.yml`, and rule R11's accepted warning).
...
8. A **non-root variant**: the upstream image runs as root and nothing in the App spec can change that. Run the
   ...
   numeric non-root `USER` owning `apps/web/.next` and `apps/web/public` (unverified). That Dockerfile would be this
   Blueprint's first overlay file; today it has none.
...
10. The `login` smoke and every readiness probe after bootstrap answer **before the ingress is published**
    ...
    server props call the auth base URL from inside the pod. `NEXTAUTH_URL_INTERNAL` (see `.works/works.yml`) keeps
    that call in-cluster; what still needs hairpin routing is the absolute links and redirects built from
...
11. Whether the in-app support address and company name (`NEXT_PUBLIC_SUPPORT_MAIL_ADDRESS`,
    ...
    need a fork-side build change for a community build — owner/counsel decision, not a platform one. The same
    applies to the in-app name, `NEXT_PUBLIC_APP_NAME`, which defaults to "Cal.diy" and has no Dockerfile ARG, so
    the running app shows the upstream's name even though the Blueprint is displayed as "Cal (community build)".
...
2. Re-read every row of the facts table at the candidate commit. Any change to the Dockerfile, the start script,
   the setup route, a cron route or `.env.example` means editing `.works/works.yml` in the same pull request. Also
   check the Dockerfile's **base image tag** and the framework's engine range (the pin builds on a floating
   `node:20`).
...
`prisma migrate deploy`, the app-store seed and `next start`. `.works/works.yml` is the whole definition; this
README is the maintainer's record of why each row is what it is (see _What the Blueprint decides_ and _Facts and
where they were read_ above). Upstream advises personal, non-production use (facts table), and the create form
shows the Blueprint's display name and trademark notice.
...
`green`. **This community build is not affiliated with or endorsed by Cal.com, Inc.**; the Blueprint's display
name is "Cal (community build)" and `license.notice` carries the trademark notice, which names the upstream's
mark, Cal.diy. The repository itself contains no trademark statement — the notice wording comes from outside it
and is confirmed by counsel before publishing (see the unverified list). The Blueprint's own repository licence is
MIT.

## Provenance

The research above was first written as a draft in the platform specification (APW-13 golden paths,
`blueprints/cal-diy/`) and consolidated here on 2026-09-26; this repository is now its only home. Since that
draft, the Blueprint id changed from `cal-diy` to `cal`, the repository was renamed from
`ever-works/cal-diy-template` to `ever-works/cal-template`, and the Blueprint's display name (and the
`EMAIL_FROM_NAME` the app sends with) changed from "Cal.diy (community build)" to "Cal (community build)". The App
spec is otherwise the one the draft describes.
